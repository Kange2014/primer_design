#
# This is the original copyright statement. I have relied on Rob Edwards's
# module extensively for this module.
#
# Copyright (c) 1997-2001 bioperl, Rob Edwards. All Rights Reserved.
...
#
# But I have modified some of it, so I guess I should add:
#
# Copyright (c) 2013 bioperl, Lulu Zheng. All Rights Reserved.
#           This module is free software; you can redistribute it and/or
#           modify it under the same terms as Perl itself. 
#
# Copyright Lulu Zheng
#
# You may distribute this module under the same terms as perl itself
# POD documentation - main docs before the code
...
Design - Create input for and work with the output 
from the program primer3

=head1 SYNOPSIS

Design creates the input files needed to design primers
...
http://primer3.sourceforge.net/releases.php for details and to 
download the software. 

This module is based on one written by Rob Edwards. I have modified some
of his code.
...
Based heavily on work of Rob Edwards
...
=head2 run()
 ...
 Returns : A string that contains the results, separated by comma.
 Args    : None.
...
sub run {
  ...
  if ( $executable =~ m{^[^\'\"]+(.+)[^\'\"]+$} ) { 
	...
	###print "There were ", $self->{results_obj}->number_of_results, " primers\n";
	
	my $id = $self->{'seqobject'}->id;
	...
	for (my $i = 0 ; $i < $self->{results_obj}->number_of_results ; $i++){
		my $result = $self->{results_obj}->primer_results($i);
		...
		for my $key qw(PRIMER_LEFT PRIMER_RIGHT){
			...
			$result->{$key . "_START"} = $start+1;####+$gstart;
			$result->{$key . "_LENGTH"} = $length;
			foreach my $partkey (@out_keys_part) {
				$line .= ",".$result->{$key . "_" . $partkey};
			} 
			$line .= "\n";
		}
		###print OUT "\tPRODUCT SIZE: ", $result->{'PRIMER_PAIR_PRODUCT_SIZE'}, ", PAIR ANY COMPL: ",
		 ## $result->{'PRIMER_PAIR_COMPL_ANY'};
		###print OUT ", PAIR 3\' COMPL: ", $result->{'PRIMER_PAIR_COMPL_END'}, "\n";
	}
	return $line;
